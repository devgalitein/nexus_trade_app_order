...
exports.addStrategy = async (req, res) => {
  try {
    ...
    if (setting_id === "") {
      sql.query(
        `INSERT INTO setting (entry_time,exit_time,auto_repeat,exit_bn_profit,exit_bn_loss,total_quantity,order_type,sl_value,created_at) VALUES ('${entry_time}', '${exit_time}', ${auto_repeat}, '${exit_bn_profit}', ${exit_bn_loss}, ${total_quantity}, '${order_type}', ${
...
exports.getOrder = async (callback) => {
  ...
  sql.query(querySelect, function (err, data, fields) {
    ...
    let orderData = data.map(async (items) => {
      ...
      console.log(
        "quoteval?.last_price quoteval?.last_price",
        quoteval?.last_price,
        quote,
        code,
        entrystrike
...
exports.addOrder = async (req, res) => {
  try {
    ...
    sql.query(
      ...
      async (err, result) => {
        console.log("resultresultresultresultresult", result, err, leg);
        if (result && result.length <= 0) {
          ...
          let quote = await getQuotes([code]);
          console.log("quotequotequotequotequote", quote, code);
          let quoteval = quote ? quote[code] : null;
          if (quoteval) {
            ...
            const data = sql.query(
              `SELECT orders.order_live_id FROM orders WHERE orders.exit_price IS NULL AND orders.created_at ='${moment().format(
                "YYYY-MM-DD"
              )}'`
            );
            // const getOrderData = await getOrderdata(place_order.order_id);
            console.log(
              "getOrderDatagetOrderDatagetOrderDatagetOrderData",
              data
            );
          }
        }

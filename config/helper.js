...
const getOrdersLegs = async () => {
  return new Promise((resolve, reject) => {
    sql.query(
      `SELECT DISTINCT legs.*,setting.entry_time,setting.order_type,setting.exit_time,setting.auto_repeat FROM legs LEFT JOIN orders on legs.id=orders.leg_id LEFT JOIN setting ON legs.setting_id=setting.id WHERE orders.created_at != '${moment().format(
        "YYYY-MM-DD"
